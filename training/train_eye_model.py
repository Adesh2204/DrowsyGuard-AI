...
from augmentations import get_eye_train_transforms, get_eye_val_transforms
from dataset_loader import EyeStateDataset
...
from app.models.eye_model import EYE_LABELS, build_eye_model  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the ResNet-50 eye-state classifier.")
    parser.add_argument("--data-dir", type=Path, default=ROOT_DIR / "data" / "eye_state")
    parser.add_argument("--output-dir", type=Path, default=ROOT_DIR / "training" / "outputs" / "eye_model")
    parser.add_argument("--weights-output", type=Path, default=ROOT_DIR / "backend" / "weights" / "eye_classifier.pth")
...
def save_confusion_matrix(cm: np.ndarray, output_path: Path) -> None:
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=EYE_LABELS, yticklabels=EYE_LABELS)
...
def main() -> None:
    ...
    full_dataset = EyeStateDataset(root_dir=args.data_dir)
    ...
    train_dataset = EyeStateDataset(
        ...
        transform=get_eye_train_transforms(image_size=64),
        samples=train_samples,
    )
    val_dataset = EyeStateDataset(
        root_dir=args.data_dir,
        transform=get_eye_val_transforms(image_size=64),
        samples=val_samples,
    )
    ...
    model = build_eye_model(pretrained=True, freeze_early=True).to(device)
    ...
    if not best_targets:
        raise RuntimeError("Validation set produced no targets. Check the dataset split.")
    ...
    cm = confusion_matrix(best_targets, best_predictions, labels=list(range(len(EYE_LABELS))))
